...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/deleted-posts/user/:userId", async (req, res) => {
    ...
    console.log(`[DELETED POSTS API] ${timestamp} - Getting deleted posts for user ${userId}`);
    ...
    try {
      if (!userId || userId.trim() === '') {
        ...
        return res.status(400).json({ error: 'Invalid user ID', timestamp });
      }
      ...
      try {
        ...
        console.log(`[DELETED POSTS API] Sample of returned data:`, deletedPosts.slice(0, 1));
