...
export interface IStorage {
  ...
  cleanupApplicationsForJob(jobId: string, userId: string): Promise<void>;
...
export class MemStorage implements IStorage {
  ...
  // Clean up applications for a specific job-user combination
  async cleanupApplicationsForJob(jobId: string, userId: string): Promise<void> {
    console.log(`[CLEANUP] Cleaning applications for job ${jobId} and user ${userId}`);

    // Find all applications for this job-user combination
    ...
    applicationsToRemove.forEach(appId => {
      ...
      console.log(`[CLEANUP] Removed application ${appId}`);
    });

    console.log(`[CLEANUP] Removed ${applicationsToRemove.length} applications for job ${jobId} and user ${userId}`);
  }
  ...
  async addDeletedPost(deletedPost: any): Promise<void> {
